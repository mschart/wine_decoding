...
See README.md for a list of all figures.
...
def get_best_data(vintage=False, best=4, rand=False, wines=None):
    ...
    for chem_type in ['oak']:  # ['esters','oak','offFla']
        ...
        if vintage:
            leasts3 = np.load(pth_dat / 'res'
                              f'/leasts_vintage_{chem_type}.npy',
                              allow_pickle=True).flat[0]
        else:
            leasts3 = np.load(pth_dat / f'res/leasts_{chem_type}.npy',
                              allow_pickle=True).flat[0]
...
def get_weights(manual=True, decoder='LDA'):
    ...
    if manual:
        s = pth_dat / f'res/LDA_weights_manual_{decoder}.npy'
    else:
        s = pth_dat / 'res/LDA_weightsS11.npy'
...
def chunk_survival(chem_type, loosers=False, vintage=False):
    ...
    If losers = True, the data chunk is removed,
    whose removal resulted in the lowest performance
    '''
    ...
    print(leasts)
    if loosers:
        np.save(pth_dat / f'res/leasts_vintage_{vintage}'
                f'_loosers_{chem_type}.npy', leasts)
    else:
        if vintage:
            p = pth_dat / f'res/leasts_vintage_{chem_type}.npy'
        else:
            p = pth_dat / f'res/leasts_{chem_type}.npy'
        np.save(p, leasts)
...
def batch_chunk2_m_parallel(shuf, vintage, cla='LDA'):
    ...
    p = pth_dat / f'res/c2_m_v{vintage}_s{shuf}_{cla}.npy'
    np.save(p, R)
...
def plot_best_concat():
    ...
    for variable in cls:
        ...
        add_panel_letter(k)
        if variable == 'vintage':
            R = np.load(pth_dat / 'res/best_vintage.npy',
                        allow_pickle=True).flat[0]
        else:
            R = np.load(pth_dat / 'res/best.npy',
                        allow_pickle=True).flat[0]
    ...
    R = np.load(pth_dat / 'res/rand_vintage.npy',
                allow_pickle=True).flat[0]
...
def plot_violin(dr=False):
    ...
    for manual in [False, True]:
        ...
        for variable in nd:
            ...
            qq = (pth_dat / f'res/{"m" if manual else "R"}'
...
def plot_chunks2(n_chunks=50.0, vintage=False):
    ...
    '''
    if vintage:
        R = np.load(pth_dat / f'res/R_chunks2_vintage_{n_chunks}.npy',
                    allow_pickle=True).flat[0]
        variable = 'vintage'
    else:
        R = np.load(pth_dat / f'res/R_chunks2_{n_chunks}.npy',
                    allow_pickle=True).flat[0]
        variable = 'estate'
...
def plot_chunks_m_dists(cla='LDA', sigl=0.01):
    ...
    rs = pth_dat / 'data/32_compound_names.npy'
    ...
    for target in ['estate', 'vintage']:
        ...
        sns.violinplot(x="compound", y="accuracy [%]", hue="shuf",
                       data=df, inner=None, ax=axs[i],
                       palette='Greys', split=True,
                       meanline=True, bw=.1)
...
def plot_vintage_decoding_per_wine():
    ...
    R = np.load(pth_dat / 'res/best_20percent_oak_vintage.npy',
                allow_pickle=True).flat[0]
...
def plot_weights():
    ...
    D0 = np.load(pth_dat / 'res/LDA_weightsS11.npy',
                 allow_pickle=True).flat[0]
    D1 = np.load(pth_dat / 'res/Ridge_weightsS11.npy',
                 allow_pickle=True).flat[0]
